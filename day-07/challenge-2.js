...
let targetSize = computer.size - (MAX_FILESYS-TARGET_FREE);
let closestDir;
function bstDirectory(root) {
  if (root.size > targetSize) {
    if (closestDir === undefined) {
      closestDir = root;
    } else {
      if (root.size < closestDir.size) {
        closestDir = root;
      }
    }
  }
  root.directories.forEach(d => bstDirectory(d));
}

bstDirectory(computer);
console.log(`The Closest Size Needed: ${closestDir.size}`);